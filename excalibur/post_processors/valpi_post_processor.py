...
class ValpiPostProcessor(PostProcessor):
    ...
    service_regex = [
        re.compile("dias úteis", re.IGNORECASE),
    ...
    def process(self, df):
        ...
        ignores_dict = {
            word: "" for word in self.ignore_words + data_frame_utils.ignore_expressions
        }
    ...
    def _create_route_name(self, series):
        ...
        self.route = "".join(
            pd.Series(series.values[mask], series.index[mask])
        ).strip()  # format route row

    def _create_lines(self, df):
        lines = df[
            df.applymap(lambda x: bool(re.search("LINHA", x))).any(axis="columns")
        ].iloc[
            0
    ...
    def _slice_dataframe(self, df, services_df, lines):
        ...
        if (
            service == "None"
        ):  # if no services were found return None with the entire df
            return [("None", df)]
        ...
        df_list = self._create_df_list(
            df, services_df
        )  # create the list of sliced dataframes
